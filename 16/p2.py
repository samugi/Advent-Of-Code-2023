...
# brute force with no tricks just worked ¯\_(ツ)_/¯
...
with open(sys.argv[1]) as f:
  ...
  for x in range(len(matrix[0])):
    for y in range(len(matrix)):
      # going down
      if y == 0:
        energized = energize(matrix, x, -1, x, y, {})
        if energized > max_energy:
          max_energy = energized
      # going up
      if y == len(matrix) - 1:
        energized = energize(matrix, x, len(matrix), x, y, {})
        if energized > max_energy:
          max_energy = energized
      # going right
      if x == 0:
        energized = energize(matrix, -1, y, x, y, {})
        if energized > max_energy:
          max_energy = energized
      # going left
      if x == len(matrix[0]) - 1:
        energized = energize(matrix, len(matrix[0]), y, x, y, {})
        if energized > max_energy:
          max_energy = energized
